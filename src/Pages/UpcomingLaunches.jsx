...
const UpcomingLaunches = () => {
  const [upcomingLaunchData, setUpcomingLaunchData] = useState('')

  useEffect(() => {
    const fetchUpcomingLaunchData = async () => {
      ...
      {!upcomingLaunchData ? (
        ...
        <article className='py-32 max-width'>
          ...
          <div className="max-width grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3 px-5">
            {upcomingLaunchData.map(({id, name, links, details, flight_number, date_local}) => (
